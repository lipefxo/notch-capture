...
final class PanelTransitionPolicyTests: XCTestCase {
    ...
    func testFocusedVolumeSurfaceMorphsWithoutBecomingAKeyboardState() {
        let opening = PanelTransitionPolicy.resolve(
            from: .collapsed,
            to: .volume,
            wasVisible: true,
            reduceMotion: false
        )
        let closing = PanelTransitionPolicy.resolve(
            from: .volume,
            to: .collapsed,
            wasVisible: true,
            reduceMotion: false
        ...
        XCTAssertEqual(opening.kind, .expand)
        XCTAssertEqual(closing.kind, .contract)
        XCTAssertTrue(PanelState.volume.isExplicitSession)
        XCTAssertFalse(PanelState.volume.acceptsKeyboardInput)
    }
...
final class NotchMotionTokenTests: XCTestCase {
    ...
    func testFastEaseOutDurationsRemainResponsive() {
        ...
        XCTAssertEqual(NotchMotion.musicTrackSwapDuration, 0.20)
        XCTAssertEqual(NotchMotion.musicTrackSwapOffset, 6)
        XCTAssertEqual(NotchMotion.musicTrackSwapBlurRadius, 2)
